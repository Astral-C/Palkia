...
UPalkiaApplication::UPalkiaApplication() {
	mWindow = nullptr;
	mContext = nullptr;
}

bool UPalkiaApplication::Setup() {
	// Initialize GLFW
	if (!glfwInit())
		return false;
	...
	mWindow = glfwCreateWindow(1280, 720, "Palkia", nullptr, nullptr);
	if (mWindow == nullptr) {
		glfwTerminate();
		return false;
	}
	...
	mContext = new UPalkiaContext();
	ResizeContext = mContext;

	return true;
}

bool UPalkiaApplication::Teardown() {
...
bool UPalkiaApplication::Execute(float deltaTime) {
